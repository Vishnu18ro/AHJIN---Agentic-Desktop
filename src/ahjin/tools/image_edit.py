"""ImageEditTool ΓÇö Resize, compress, or manipulate image files."""
    ...
    async def execute(self, request: ToolInvocationRequest) -> ToolInvocationResult:
        ...
        try:
            with Image.open(target_file) as img:
                ...
                elif operation == "enlarge" and target_kb and isinstance(target_kb, (int, float)):
                    target_bytes = target_kb * 1024
                    
                    # First try maximum quality
                    buffer = io.BytesIO()
                    if original_format == "JPEG":
                        processed_img.save(buffer, format=original_format, quality=100, optimize=False, subsampling=0)
                    else:
                        processed_img.save(buffer, format=original_format)
                    
                    # If still too small, iteratively scale up the resolution
                    scale_factor = 1.0
                    current_img = processed_img
                    while buffer.tell() < target_bytes and scale_factor < 5.0:
                        scale_factor += 0.2
                        ...
                        buffer = io.BytesIO()
                        if original_format == "JPEG":
                            current_img.save(buffer, format=original_format, quality=100, optimize=False, subsampling=0)
                        else:
                            current_img.save(buffer, format=original_format)
