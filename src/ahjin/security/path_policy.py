            ...
            if platform.system() == "Windows":
                ...
                try:
                    with winreg.OpenKey(winreg.HKEY_CURRENT_USER, r"Software\Microsoft\Windows\CurrentVersion\Explorer\Shell Folders") as k:
                        ...
                        if desktop:
                            candidate_folders.append(Path(desktop).resolve())
                        if documents:
                            candidate_folders.append(Path(documents).resolve())
                        if downloads:
                            candidate_folders.append(Path(downloads).resolve())
                except Exception:
                    pass
    ...
    def resolve_shortcut(self, target_str: str) -> Path | None:
        ...
        for keys, folder_name in aliases:
            if clean in keys or clean.endswith(folder_name.lower()):
                ...
                candidates: list[Path] = [home / folder_name, onedrive / folder_name]
                if platform.system() == "Windows":
                    import winreg
                    try:
                        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, r"Software\Microsoft\Windows\CurrentVersion\Explorer\Shell Folders") as k:
                            if folder_name.lower() == "desktop":
                                candidates.append(Path(winreg.QueryValueEx(k, "Desktop")[0]).resolve())
                            elif folder_name.lower() == "documents":
                                candidates.append(Path(winreg.QueryValueEx(k, "Personal")[0]).resolve())
                            elif folder_name.lower() == "downloads":
                                candidates.append(Path(winreg.QueryValueEx(k, "{374DE290-123F-4565-9164-39C4925E467B}")[0]).resolve())
                    except Exception:
                        pass
